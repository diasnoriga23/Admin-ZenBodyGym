    ...
    <div class="animate__animated animate__fadeIn content position-relative">
        <div class="container my-4">
            <div class="col-4 semi">
                <div class="col my-1"><h2 class="my-0">Buku Pengunjung</h2></div>
            </div>
            <hr>
        </div>
        <div class="container" style="top: 300px; width: 900px;">
            <div class="col-12 col-dsb-bg my-4 py-3">
                ...
                <div class="row">
                <div class="col-6 med">
                    <div class="col my-1"><h2 class="my-0">Pengunjung</h2></div>
                </div>
                <div class="col reg">
                <form method="POST" action="lihatpengun.php">
                <label for="tgl" class="form-label">Tangal</label>
                    <input name="tgl" type="text" class="form-control" id="tgl" required>
                </div>
                <div class="col">
                    <div class="col d-flex justify-content-end align-items-end h-100 reg">
                        <button name="lihatPengun" type="submit" class="btn btn-primary">Lihat</button>
                    </div>
                    </form> 
                </div>
            </div>
            <hr>
                    ...
                    <?php
                    if(isset($_POST['lihatPengun'])){

                  
                        $hariini           = $_POST['tgl'];
                        $reference_tamu = 'Pengunjung/'.$hariini;
                        $checkdata_tamu = $database->getReference($reference_tamu)->getValue();
                        if($checkdata_tamu != null){
                        foreach($checkdata_tamu as $value){
                   ?>
                    <tbody class="reg">                    
                        <tr>
                            <td class="bank"><?php echo $value['nama']; ?></td>
                            <td><?php echo $value['alamat']; ?></td>
                        </tr>
                    </tbody>
                    <?php } }else{
                        echo "Tidak Ada Pengunjung";
                    }}?>
                </table>
                </div>
            </div>
